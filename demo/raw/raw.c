...
//no global context! need to use nng_ctx_send!
void
server_cb(void *arg)
{
	...
	//use nng_ctx_send? nng_ctx_send(work->ctx, work->aio);
	switch (work->state) {
	case INIT:
		printf("INIT ^^^^^^^^^^^^^^^^^^^^^ \n");
		work->state = RECV;
...
// The server runs forever. father process
int
...
client(const char *url, const char *msecstr)
{
	...
	const char tmp[20]="12345678901234567890";
	if ((rv = nng_req0_open_raw(&sock)) != 0) {
	...
	*/
	...
	//nng_send(sock, tmp, strlen(tmp), 0);
